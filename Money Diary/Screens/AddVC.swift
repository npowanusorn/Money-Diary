...
class AddVC: UIViewController {
    ...
    @IBAction func addButtonTapped(_ sender: Any) {
        ...
        if let checkImage = UIImage(systemName: "checkmark.circle.fill")?.withTintColor(.systemBlue, renderingMode: .alwaysTemplate) {
            SPIndicator.present(title: "Saved", preset: .custom(checkImage), haptic: .success)
//            let indicatorView = SPIndicatorView(title: "Saved", message: nil, preset: .custom(checkImage))
//            indicatorView.present(duration: 1.0, haptic: .success, completion: nil)
        } else {
            let indicatorView = SPIndicatorView(title: "Saved", message: nil, preset: .done)
            indicatorView.present(duration: 1.0, haptic: .success, completion: nil)
        }
        dismissVC()
    }
    ...
    func setupContentViewTap() {
        contentView.isUserInteractionEnabled = true
        let tap = UITapGestureRecognizer(target: self, action: #selector(handleContentViewTap))
        contentView.addGestureRecognizer(tap)
    }

    @objc
    func handleContentViewTap(_ gesture: UITapGestureRecognizer) {
